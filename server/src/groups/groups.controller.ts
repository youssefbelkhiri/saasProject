import { Controller, Get, Post,Patch, Delete, Param, Body, Request, UseGuards, HttpException } from '@nestjs/common';
...
export class GroupsController {
  ...
  async createGroup(@Body() groupDto: CreateGroupDto, @Request() req){
    if(req.user.id === groupDto.user_id){
  ...
  @Patch("/:id")
  async updateGroup(@Param("id") id: number, @Body() groupDto: UpdateGroupDto, @Request() req){
    const userGroup = await this.groupsService.getGroup(id);
